...
- Ship **one binary** — `UsageCheck` (`com.usagecheck.desktop`) — with a **runtime license unlock**: everyone gets Codex/Claude/agy, and a Pro license key entered in the tray unlocks Cursor/Grok/Higgsfield without reinstalling. Build with `./scripts/build-edition.sh [tauri bundle args...]`. (This supersedes the earlier two-binary `UsageCheck-Free`/`UsageCheck-Pro` preference, which was replaced when the license gate landed; the compile-time `edition-free`/`edition-pro` Cargo features and `tauri.pro.conf.json` no longer exist.)
...
- **Free for everyone**: Codex, Claude, agy (Gemini via Antigravity quota API). **Pro-gated at runtime** by a license key: Cursor (local `state.vscdb` + undocumented `GetCurrentPeriodUsage`), Grok (xAI Management prepaid via clipboard/env `XAI_MGMT_KEY`/`XAI_TEAM_ID`), Higgsfield (CLI `account --json` / tray browser login). Unlicensed paid accounts are kept, never deleted, and shown as status `pro_required`. Activation is an **online** call to autoworkit.com that returns an **Ed25519-signed token**; the token is persisted and its signature re-verified on every status check, so a hand-edited cache fails closed to Free. Wire contract: `docs/LICENSE_API.md`. CI Release workflow builds a 2-job matrix (`UsageCheck-macos`, `UsageCheck-windows`) on `v*` tags and publishes 3 assets (dmg, exe, msi); a `guard` job fails the release while the embedded license public key is still the placeholder. See `docs/editions.md`.
