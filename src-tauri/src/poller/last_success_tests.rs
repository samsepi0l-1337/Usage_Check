...
fn auth_source_evict_is_isolated() {
    ...
    let (y_result, x_result) = {
        let mut cache = last_success_cache()
            .lock()
            .unwrap_or_else(|error| error.into_inner());
        let y_result = apply_last_success(&mut cache, "y", auth_source_usage("y", "error", None));
        let x_result = apply_last_success(&mut cache, "x", auth_source_usage("x", "error", None));
        (y_result, x_result)
    };
...
fn auth_source_ok_then_throttled_serves_stale() {
    ...
    let mut good = auth_source_usage("account-1", "ok", Some(auth_source_quota(25.0)));
    good.week = Some(auth_source_quota(50.0));
    good.pool_breakdown = vec![usage_core::fetch::agy::AgyQuotaPool {
        name: "Gemini".into(),
        five_hour: Some(auth_source_quota(25.0)),
        week: Some(auth_source_quota(50.0)),
    }];
    good.detail_suffix = Some("$12 left".into());
    apply_last_success(&mut cache, "account-1", good.clone());
    ...
    assert_eq!(result.status, "stale");
    assert_eq!(result.five_hour, good.five_hour);
    assert_eq!(result.week, good.week);
    assert_eq!(result.plan, good.plan);
    assert_eq!(result.pool_breakdown, good.pool_breakdown);
    assert_eq!(result.detail_suffix, good.detail_suffix);
}
